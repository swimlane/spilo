...
# use pip instead of ubuntu pkgs
# requirements were sourced from the pip freeze of a spilo container and updated using pip-tools
pip3 install -r /builddeps/requirements.txt
...
if [ "$DEMO" != "true" ]; then
    ...
    sed -i 's/^\(    for i in range(0,\) num_retries):.*/\1 100):/g' /usr/local/lib/python3.10/dist-packages/boto/utils.py
else
    EXTRAS=""
fi
